...
function isAlreadyFinalStatus(validasiList) {
...
function canRejectAtLevel(validasiList, currentLevel) {
...
export async function rejectBatchForUser(batchId, user, catatan) {
    ...
    for (const mhs of mahasiswaEligible) {
        const existing = await findValidasiByMahasiswaAndLevel(mhs.id_mahasiswa, approvalLevel);
        if (existing) {
    ...
    await createLogAktivitas({
        ...
        deskripsi: `${user.role} menolak batch ${batch.nomor_batch_upload ?? batch.id_batch_upload} pada level ${approvalLevel}. Total mahasiswa ditolak: ${results.length}`,
    });
    return {
        batch: {
...
//# sourceMappingURL=reject-batch.service.js.map