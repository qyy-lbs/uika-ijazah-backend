...
function isAlreadyRejectedOrRevoked(validasiList) {
    return validasiList.some((item) => {
        const status = item.status_validasi?.toLowerCase();
        return (status === VALIDATION_STATUS.REJECTED ||
            status === VALIDATION_STATUS.REVOKED);
    });
}
function canRevokeAtLevel(validasiList, currentLevel) {
...
export async function revokeMahasiswaForUser(mahasiswaId, user, catatan) {
    ...
    const existing = await findValidasiByMahasiswaAndLevel(mahasiswa.id_mahasiswa, approvalLevel);
    ...
    await createLogAktivitas({
        ...
        deskripsi: `${user.role} merevoke mahasiswa ${mahasiswa.nim} - ${mahasiswa.nama_mahasiswa ?? "-"} pada level ${approvalLevel}. Alasan: ${catatan}`,
    });
    return {
        mahasiswa: {
...
//# sourceMappingURL=revoke-mahasiswa.service.js.map