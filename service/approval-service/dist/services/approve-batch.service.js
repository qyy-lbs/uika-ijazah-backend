...
function isRevokedOrRejected(validasiList) {
    return validasiList.some((item) => {
        ...
        return (status === VALIDATION_STATUS.REVOKED ||
            status === VALIDATION_STATUS.REJECTED);
    });
}
function canApproveAtLevel(validasiList, currentLevel) {
...
export async function approveBatchForUser(batchId, user) {
    ...
    await createLogAktivitas({
        ...
        deskripsi: `${user.role} memvalidasi batch ${batch.nomor_batch_upload ?? batch.id_batch_upload} pada level ${approvalLevel}. Total mahasiswa divalidasi: ${results.length}`,
    });
...
//# sourceMappingURL=approve-batch.service.js.map