import { getBatchDashboardRepository, getDetailBatchRepository, getBatchRepository, } from "../repositories/batch.repository.js";
...
export const getDetailBatchService = async (id, status) => {
    const rows = (await getDetailBatchRepository(id));
    if (!rows.length) {
        return null;
    }
    const requestedStatus = status ? normalizeFilterStatus(status) : "";
    let mahasiswa = rows
        .filter((item) => item.id_mahasiswa)
        .map((item) => {
...
export const getBatchService = async (page, limit, tahun_lulus, periode, search, status) => {
    const result = await getBatchRepository(page, limit, tahun_lulus, periode, search, status);
