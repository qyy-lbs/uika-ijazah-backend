...
function formatTempatTanggalLahir(tempat, tanggal) {
    ...
    if (!tempat && !tanggalFormatted)
        return null;
    return `${tempat || "-"}, ${tanggalFormatted || "-"}`;
}
function getNextValidatorLabel(lastApprovedLevel) {
    const nextLevel = lastApprovedLevel + 1;
    return APPROVAL_LEVEL_LABEL[nextLevel] ?? "Selesai";
}
function getMahasiswaApprovalStatus(validasiList) {
    const rejected = validasiList.find((item) => item.status_validasi?.toLowerCase() === "rejected");
    if (rejected) {
        return {
            status: "Reject",
            keterangan: `Di Reject oleh ${APPROVAL_LEVEL_LABEL[rejected.level_validasi]}`,
            deskripsi: rejected.catatan || "Data ditolak karena tidak sesuai persyaratan.",
            level_validasi: rejected.level_validasi,
            catatan: rejected.catatan,
            validated_at: rejected.validated_at,
        };
    }
    const revoked = validasiList.find((item) => item.status_validasi?.toLowerCase() === "revoked");
    if (revoked) {
        return {
            status: "Revoke",
            keterangan: `Di Revoke oleh ${APPROVAL_LEVEL_LABEL[revoked.level_validasi]}`,
            deskripsi: revoked.catatan || "Mahasiswa dikeluarkan dari proses validasi.",
    ...
    return {
        ...
        deskripsi: "Data sedang dalam proses verifikasi. Mohon menunggu hingga proses validasi selesai.",
        level_validasi: lastApprovedLevel,
        catatan: null,
        validated_at: lastApproved?.validated_at ?? null,
    };
}
export async function getProfileByMahasiswaCode(mahasiswaCode) {
    ...
    const approval = getMahasiswaApprovalStatus(mahasiswa.validasi.map((v) => ({
        level_validasi: v.level_validasi,
        status_validasi: v.status_validasi,
        catatan: v.catatan,
        validated_at: v.validated_at,
    })));
    const tempatTanggalLahir = formatTempatTanggalLahir(mahasiswa.tempat_lahir, mahasiswa.tanggal_lahir);
    return {
        mahasiswa: {
            ...
            tanggal_kelulusan_formatted: formatTanggalIndonesia(mahasiswa.tanggal_kelulusan),
