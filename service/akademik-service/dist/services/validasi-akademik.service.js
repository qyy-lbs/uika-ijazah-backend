...
export async function getValidasiAkademikByMahasiswaId(mahasiswaId) {
    ...
    const errors = [];
    if (!checks.data_mahasiswa)
        errors.push("Data mahasiswa belum lengkap");
    if (!checks.prodi)
        errors.push("Program studi belum tersedia");
    if (!checks.fakultas)
        errors.push("Fakultas belum tersedia");
    if (!checks.ipk)
        errors.push("IPK belum tersedia");
    if (!checks.tahun_lulus)
        errors.push("Tahun lulus belum tersedia");
    if (!checks.tanggal_kelulusan)
        errors.push("Tanggal kelulusan belum tersedia");
    if (!checks.nomor_seri_ijazah)
        errors.push("Nomor seri ijazah belum tersedia");
    if (!checks.nilai_transkrip)
        errors.push("Data transkrip nilai belum tersedia");
...
//# sourceMappingURL=validasi-akademik.service.js.map