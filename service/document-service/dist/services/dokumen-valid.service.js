import { findBatchesWithValidDocuments, findBatchWithValidDocumentsById, } from "../repositories/dokumen-valid.repository.js";
...
function getPublishedDocuments(mahasiswa) {
    const documents = Array.isArray(mahasiswa.dokumen) ? mahasiswa.dokumen : [];
    const ijazah = documents.find((doc) => doc.jenis_dokumen?.toLowerCase() === "ijazah" &&
        doc.is_verified === true);
    const transkrip = documents.find((doc) => doc.jenis_dokumen?.toLowerCase() === "transkrip" &&
        doc.is_verified === true);
    return {
        ijazah,
        transkrip,
...
function hasValidPublishedDocuments(mahasiswa) {
...
export async function getValidDocumentBatches(query) {
    ...
    const mapped = batches
        ...
        return {
            ...
            batch: batch.nomor_batch_upload ||
    ...
    const filtered = mapped.filter((item) => {
        ...
        const matchFakultas = !fakultas || item.fakultas?.toLowerCase() === fakultas;
...
export async function getValidDocumentBatchDetail(batchId, query) {
        ...
        return {
            ...
            ijazah: ijazah
                ? {
                    ...
                    file_pdf_url: buildFileUrl(ijazah.file_pdf_final || ijazah.file_pdf),
                }
                : null,
            transkrip: transkrip
                ? {
                    ...
                    file_pdf_url: buildFileUrl(transkrip.file_pdf_final || transkrip.file_pdf),
                }
                : null,
        };
    });
    ...
    const filteredMahasiswa = search
        ? validMahasiswa.filter((mhs) => mhs.nama?.toLowerCase().includes(search) ||
...
//# sourceMappingURL=dokumen-valid.service.js.map