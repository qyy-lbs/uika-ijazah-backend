...
function getTextJustify(element) {
    if (element.align === "left")
        return "flex-start";
    if (element.align === "right")
        return "flex-end";
...
function renderTextElement(element, profile, scaleX, scaleY, defaultFontSize) {
    ...
    if (element.label === "Gelar") {
        ...
        const gelarEn = valueToString(getValueByPath(profile, "mahasiswa.gelar_en"));
        value = [gelar, gelarEn]
            .filter(Boolean)
            .join(" / ");
    }
...
function renderImageElement(element, profile, scaleX, scaleY) {
...
function renderSignatureElement(element, profile, scaleX, scaleY) {
    const rawValue = valueToString(getValueByPath(profile, element.field));
    const imageUrl = resolvePublicAssetUrl(rawValue);
    const left = (element.x ?? 0) * scaleX;
    const top = (element.y ?? 0) * scaleY;
    ...
    const roleLabel = typeof element.roleLabel === "string" && element.roleLabel.trim()
        ? element.roleLabel
        : "Dekan,";
...
function renderQrElement(element, profile, scaleX, scaleY) {
    const rawValue = valueToString(getValueByPath(profile, element.field));
...
function renderTranskripTable(element, profile, scaleX, scaleY) {
    ...
    const renderRows = (list) => list
        ...
        return `
          <tr>
            ...
            <td class="mk-name">${escapeHtml(String(item.nama ?? item.mata_kuliah ?? ""))}</td>
        ...
        .join("");
    const renderTable = (list, withFooter) => `
  ...
  `;
    const qualificationRows = KUALIFIKASI_NILAI.map(([huruf, angka, kualifikasi]) => `
      <tr>
...
function renderElement(element, profile, scaleX, scaleY, defaultFontSize) {
...
export function renderDocumentHtml(params) {
    ...
    const pageConfig = getDocumentPageConfig(jenis, layout.imageNaturalWidth, layout.imageNaturalHeight);
    ...
    const renderedElements = elements
        .map((element) => renderElement(normalizeTemplateElement(element, jenis), params.profile, scaleX, scaleY, pageConfig.defaultFontSize))
        .join("");
    return `
    <!DOCTYPE html>
    <html>
...
//# sourceMappingURL=document-html-renderer.service.js.map