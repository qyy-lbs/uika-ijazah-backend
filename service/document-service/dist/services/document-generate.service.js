...
async function generateSingleDocument(params) {
    ...
    const profileForRender = typeof params.profile === "object" && params.profile !== null
        ? {
            ...params.profile,
            dokumen_placeholder: {
                ...(params.profile.dokumen_placeholder || {}),
                nomor_dokumen: nomorDokumen,
    ...
    const pageConfig = getDocumentPageConfig(params.jenis, layout.imageNaturalWidth ?? undefined, layout.imageNaturalHeight ?? undefined);
...
export async function generateDocumentsByNim(nim, mahasiswaCode, mahasiswaId) {
    const profile = await getAkademikProfileByNim(nim, mahasiswaCode);
    const idMahasiswaRaw = typeof mahasiswaId === "number"
        ? mahasiswaId
...
//# sourceMappingURL=document-generate.service.js.map